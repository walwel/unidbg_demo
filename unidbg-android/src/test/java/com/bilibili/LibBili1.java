...
public class LibBili1 {  // todo LibBili1 不继承自AbstractJni
    ...
    LibBili1(){
        ...
        vm = emulator.createDalvikVM(new File("unidbg-android\\src\\test\\java\\com\\bilibili\\bilibili.apk")); // 创建Android虚拟机
        vm.setDvmClassFactory(new ProxyClassFactory());  // todo vm.setJni(this);改成 vm.setDvmClassFactory(new ProxyClassFactory());
        vm.setVerbose(true); // 设置是否打印Jni调用细节
        DalvikModule dm = vm.loadLibrary(new File("unidbg-android\\src\\test\\java\\com\\bilibili\\libbili.so"), true);
        module = dm.getModule(); //
