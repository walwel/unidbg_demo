package com.sina_international;
...
public class sina extends AbstractJni{
    ...
    sina() {
        ...
        vm = emulator.createDalvikVM(new File("unidbg-android\\src\\test\\java\\com\\sina_international\\sinaInternational.apk"));
        //
//        vm = emulator.createDalvikVM(null);

        // 加载目标SO
        DalvikModule dm = vm.loadLibrary(new File("unidbg-android\\src\\test\\java\\com\\sina_international\\libutility.so"), true); // 加载so到虚拟内存
        //获取本SO模块的句柄,后续需要用它
        module = dm.getModule();
    ...
    public void patchVerify(){
        int patchCode = 0x4FF00100; // todo
        emulator.getMemory().pointer(module.base + 0x1E86).setInt(0,patchCode);  //todo  0x1E86 的计算
    }
