...
public class oasis extends AbstractJni{
    ...
    oasis() {
        ...
        vm = emulator.createDalvikVM(new File("unidbg-android\\src\\test\\java\\com\\lvzhou\\lvzhou.apk"));
        // 加载目标SO
        DalvikModule dm = vm.loadLibrary(new File("unidbg-android\\src\\test\\java\\com\\lvzhou\\liboasiscore.so"), true); // 加载so到虚拟内存
        //获取本SO模块的句柄,后续需要用它
        module = dm.getModule();
